...
# Introduction 
Purpose: Prepare a policy brief for local City Council representatives. Do households value transit-rich neighborhoods compared to others? How certain can you be about your conclusions given some of the spatial biases we’ve discussed?
...
crime_station_tracts <- st_intersection(allCrime, station_tracts10_17)
...
# Aggregate crime counts by year (2010) and stop
crime10_station_summary <-
	crime_station_tracts %>%
	st_drop_geometry() %>%
	group_by(crimeYear,GIS_ID) %>%
	summarize(Crime = sum(crimeYear=="2010")) %>%
	filter(crimeYear=="2010")

crime17_station_summary <-
	crime_station_tracts %>%
	st_drop_geometry() %>%
	group_by(crimeYear,GIS_ID) %>%
	summarize(Crime = sum(crimeYear=="2017")) %>%
	filter(crimeYear=="2017")

# Collapse yearly crime counts by stop into one column
crime_summary <- rbind(crime10_station_summary, crime17_station_summary)


# Add aggregated crime data
pop_rent_crime_summary <-
	left_join(crime_summary, pop_rent_summary, by = c("GIS_ID"="GIS_ID", "crimeYear"="year"))
...
crime_station_GeoSummary <-
	left_join(metroStops, pop_rent_crime_summary, by = "GIS_ID") %>%
	dplyr::select(-WEB_URL, -TRAININFO_URL)
